...
class Aggregator:
    def __init__(self, ns_list):
        ...
        self.pub = rospy.Publisher('aggregated_image', Image, queue_size=3)
        self.bridge = CvBridge()
            ...
            with self.lock:
                ...
                if self.capture_time+rospy.Duration(4.0) > rospy.Time.now():
                    if self.capture_time+rospy.Duration(2.0) > rospy.Time.now():
                        ...
                        self.pub.publish(self.bridge.cv2_to_imgmsg(numpy.squeeze(numpy.array(self.image_captured)), encoding="rgb8"))
                    elif self.calibrate_time+rospy.Duration(20.0) > rospy.Time.now():
                        # Succeeded optimization
                        self.pub.publish(self.bridge.cv2_to_imgmsg(numpy.squeeze(numpy.array(self.image_optimized)), encoding="rgb8"))
                        if beep_time+rospy.Duration(8.0) < rospy.Time.now():
                            beep_time = rospy.Time.now()
                            beep([(600, 63, 0.1), (800, 63, 0.1), (1000, 63, 0.3)])
                    else:
                        # Failed optimization
                        self.pub.publish(self.bridge.cv2_to_imgmsg(numpy.squeeze(numpy.array(self.image_failed)), encoding="rgb8"))
                        if beep_time+rospy.Duration(4.0) < rospy.Time.now():
                            beep_time = rospy.Time.now()
                            beep([(400, 63, 0.1), (200, 63, 0.1), (100, 63, 0.6)])

                else:
                    self.pub.publish(self.bridge.cv2_to_imgmsg(numpy.squeeze(numpy.array(self.image_out)), encoding="rgb8"))
